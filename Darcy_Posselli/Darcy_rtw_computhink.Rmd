...
dataset <- read.csv(dataset, StringAsFactor=F)
```
...
#Correlation test amongst variables

```{r}
```{r, eval=TRUE}
controls=names(dataset)[c(4:6)]

for (x in controls){
    p=cor.test(dataset[,x],dataset$Right_to_Work)
    
    if(p$p.value<0.05){
        messageYES=paste('Right to Work is correlated with',x)
        print(messageYES)
        
    }else{
        messageNO=paste('Right to Work is NOT correlated with',x)
        print(messageNO)
    }
...
```{r}
cor(dataset[controls])
```

#Logistic regression

```{r}
LogitRTW = glm(Right_to_Work ~ sex + age + incwage, 
                   data = dataset,
                   family = binomial())
